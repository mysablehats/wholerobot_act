...
#     pub.publish(js)
RESTRICTWITHRATE = False #dangerous to change to False
TESTING = False
DONTWAITFORGOAL = True # dangerous to change to True
SUPERCONTROL = True
CONTROLANDMOVE = False
...
class ControlHead:
    def __init__(self):
        ...
        self.velfiltersize = 4 # TODO: param
        self.velfilterlx = []
        self.velfilterly = []
        for i in range(0,self.velfiltersize):
            self.velfilterlx.append(0)
            self.velfilterly.append(0)

        self.posfiltersize = 2 # TODO: param
        self.posfilterlx = []
        self.posfilterly = []
        for i in range(0,self.posfiltersize):
            self.posfilterlx.append(0)
            self.posfilterly.append(0)
        ...
        try:
            ...
            print(e)
            rospy.logerr(e)
            rospy.signal_shutdown('ptu not publishing in 10 seconds. breaking out')


        rospy.logwarn('waiting for heads topic to publish')
        try:
            heads =  rospy.wait_for_message('heads', HeadsArrayFloat, timeout= 20)
        except rospy.ROSException as e:
            print(e)
            rospy.logerr(e)
            rospy.signal_shutdown('no heads found within 20 seconds. breaking out')
        ...
        self.myjs_max_velocity = tuple([ 1.1, 1.1 ])
        increase_in_angle_of_ptu_pan_to_get_from_middle_to_extreme_right = 0.35 #min([1.2/theratenum, 6.5]) ### needs to be measured. changing the rate changes this!
        increase_in_angle_of_ptu_tilt_to_get_from_middle_to_extreme_bottom = 0.28#min([0.8/theratenum, 6.5]) ### needs to be measured
        self.kvx = 0.0045
        self.kvy = 0.0027
        self.setpoint = [self.width/2.0, self.height/4.0]
        nocontrolzonex = 0.3#0133333333333
        nocontrolzoney = 0.2#0133333333333
        self.count_angles_mod = 6 ### without having a local publisher for flows, this is too fast

        ###parameters for 3
        # self.myjs_max_velocity = tuple([ 1.1, 1.1 ])
        # increase_in_angle_of_ptu_pan_to_get_from_middle_to_extreme_right = 0.25 #min([1.2/theratenum, 6.5]) ### needs to be measured. changing the rate changes this!
        # increase_in_angle_of_ptu_tilt_to_get_from_middle_to_extreme_bottom = 0.18#min([0.8/theratenum, 6.5]) ### needs to be measured
        # self.kvx = 0.0045
        # self.kvy = 0.0027
        # self.setpoint = [self.width/2.0, self.height/4.0]
        # nocontrolzonex = 0.3#0133333333333
        # nocontrolzoney = 0.2#0133333333333
        # self.count_angles_mod = 3 ### without having a local publisher for flows, this is too fast
        ...
        while not rospy.is_shutdown():
            ...
            if RESTRICTWITHRATE and not SUPERCONTROL:
                self.docontrol()
            myrate.sleep();
    ...
    def get_angles(self, js):
        ...
        #self.mylock.release()
        self.count_angles = self.count_angles +1
        if not CONTROLANDMOVE:
            self.docontrol()
        if SUPERCONTROL and not RESTRICTWITHRATE and (self.count_angles%self.count_angles_mod == 0 or self.escaping): ## will move more frequently if the subject is escaping!
            if CONTROLANDMOVE:
                self.docontrol()
                self.domove()
            else:
                self.domove()
            self.count_angles = 0
            rospy.logdebug('count_angles reset!')
        # if self.count_angles%self.count_angles_mod == 0:
        #     self.count_angles = 0
        #     rospy.loginfo('count_angles reset!')
    ...
    def updateheads(self, data):
        ...
        if self.HA:
            rospy.logdebug('haveheads. updating control variables!')
            if len(self.HA)>1:
            ...
            self.y = self.HA[0].top# (self.HA[0].top+self.HA[0].bottom)/2
            if self.y < 10: ### this should check all corners, but the most common one is the top...
                self.escaping = True
            else:
                self.escaping = False
            ...
            self.velx = self.kvx*deltax
            self.vely = self.kvy*deltay
            rospy.logdebug("change {}, {}".format(self.changex, self.changey))
            rospy.logdebug("vel {}, {}".format(self.velx, self.vely))

            if not RESTRICTWITHRATE and not SUPERCONTROL:
                self.docontrol()
        #print('gotheads')

    def docontrol(self):
        if self.myjs and self.HA:
            beforetime = rospy.get_time()
            rospy.logdebug('Entering control.')
            ### now I want changes in velocity to be smooth
            ### so this is a filter. I can probably use a better filter here.
            ### TODO: better filter

            self.posfilterlx.append(self.changex)
            self.posfilterly.append(self.changey)
            self.posfilterlx.pop(0)
            self.posfilterly.pop(0)
            self.changex = np.mean(self.posfilterlx)
            self.changey = np.mean(self.posfilterly)
            rospy.logdebug("pos after mean: {}, {}".format(self.changex, self.changey))

            self.velfilterlx.append(self.velx)
            self.velfilterly.append(self.vely)
            self.velfilterlx.pop(0)
            self.velfilterly.pop(0)
            self.velx = abs(np.mean(self.velfilterlx))
            self.vely = abs(np.mean(self.velfilterly))
            rospy.logdebug("vel after mean and modulus: {}, {}".format(self.velx, self.vely))
            ...
            try:
                ...
                for jointName, jointPosition, jointVelocity in zip(self.myjs.name,self.myjs.position, velocities):
                    ...
                    if jointName == 'ptu_pan':
                        ...
                        if self.x>self.upperbound[0] or self.x<self.lowerbound[0]:
                            ...
                            rospy.logdebug("velx {}, used_velx{}".format(self.velx, jointVelocity))
                    ...
                    if jointName == 'ptu_tilt':
                        ...
                        if self.y>self.upperbound[1] or self.y<self.lowerbound[1]:
                            ...
                            rospy.logdebug("vely {}, used_vely{}".format(self.vely, jointVelocity))
                ...
                #rospy.logdebug(self.myjs.position)
                #rospy.logdebug(self.myjs)
            ...
            now = rospy.get_time()
            rospy.logdebug("Time elapsed:{}".format(now-beforetime))
            with self.mylock:
...
#set_angles(           float(sys.argv[1]),            float(sys.argv[2]))
    def domove(self):
        rospy.logdebug("entering domove")
