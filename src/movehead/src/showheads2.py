    ...
    def pubimage(self):
        ...
        for aHead in self.HA:
            if firsthead:
                ...
                headtext = '?'
            cv2.rectangle(self.bg,(int(aHead.left),int(aHead.top)),(int(aHead.right),int(aHead.bottom)),headcolor,3)
            cv2.putText(self.bg,headtext,(int(aHead.left),int(aHead.bottom)),self.font,1,(255,255,255))
