...
class ControlHead:
    def __init__(self):
        ...
        try:
            lastjs =  rospy.wait_for_message('/ptu/joint_states', JointState, timeout= 10)
            rospy.logwarn(lastjs)
        ...
        lowerbound = [self.setpoint[0] - self.width/6, self.setpoint[1] - self.height/6]
        upperbound = [self.setpoint[0] + self.width/6, self.setpoint[1] + self.height/6]
        rospy.loginfo('lowerbound {},{}\nupperbound {},{}\nsetpoint {},{}'.format(lowerbound[0], lowerbound[1], upperbound[0],upperbound[1], self.setpoint[0], self.setpoint[1]))
                ...
                if self.newdata:
                    ...
                    for jointName, jointPosition in iterset:
                        ...
                        if jointName == 'ptu_pan':
                            ...
                            if self.x>upperbound[0]:
                                print("decreasing x")
                                jointPosition += -.2
                            elif self.x<lowerbound[0]:
                                print("increasing x")
                                jointPosition += .2
                            else:
                                print("x unaltered")
                            if self.x>upperbound[0] or self.x<lowerbound[0]:
                                print("controlling!")
                        ...
                        if jointName == 'ptu_tilt':
                            ...
                            if self.y>upperbound[1]:
                                print("decreasing y")
                                jointPosition += -.2
                            elif self.y<lowerbound[1]:
                                print("increasing y")
                                jointPosition += .2
                            else:
                                print("y unaltered")
                            if self.y>upperbound[1] or self.y<lowerbound[1]:
                                print("controlling!")
