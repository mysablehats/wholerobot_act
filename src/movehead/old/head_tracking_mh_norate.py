...
class ControlHead:
    def __init__(self):
        ...
        self.setpoint = [self.width/2, self.height/2.5]

        self.lowerbound = [self.setpoint[0] - self.width/6, self.setpoint[1] - self.height/6]
        self.upperbound = [self.setpoint[0] + self.width/6, self.setpoint[1] + self.height/6]
        rospy.loginfo('lowerbound {},{}\nupperbound {},{}\nsetpoint {},{}'.format(self.lowerbound[0], self.lowerbound[1], self.upperbound[0],self.upperbound[1], self.setpoint[0], self.setpoint[1]))
        #rospy.signal_shutdown('just checking constants')

        self.myjs_velocity = tuple([ 0.29, 0.29 ])
    ...
    def updateheads(self, data):
        ...
        if data.heads:
            ...
            self.x = (data.heads[0].left+data.heads[0].right)/2
            self.y = (data.heads[0].top+data.heads[0].bottom)/2
            #calculate the error for the controller
            deltax = self.x - self.setpoint[0]
            deltay = self.y - self.setpoint[1]
            rospy.loginfo("Camerainfo is (%d,%d). face detected at %d,%d"%(self.width,self.height,self.x,self.y))
            rospy.loginfo("deltax {}, deltay {}".format(deltax,deltay))
            self.changex = -self.kx*deltax
            self.changey = -self.ky*deltay
            print("change {}, {}".format(self.changex, self.changey))

            try:
                mypositions = []
                self.beforecontroljs = deepcopy(self.myjs)
                rospy.logwarn('have heads')
                for jointName, jointPosition in zip(self.myjs.name,self.myjs.position):
                    oldjp =  jointPosition
                    print('jointName is: {}'.format(jointName))
                    if jointName == 'ptu_pan':
                        print('doing ptu_pan stuff')
                        print(jointName + 'lowerbound {}, upperbound {}'.format(self.lowerbound[0], self.upperbound[0]))
                    ...
                    if jointName == 'ptu_tilt':
                        ...
                        print(jointName + 'lowerbound {}, upperbound {}'.format(self.lowerbound[1], self.upperbound[1]))
                ...
                self.myjs.velocity = self.myjs_velocity
                ...
                if all(abs( jointPosition - jointGoal) < 0.05 for jointPosition, jointGoal in zip(self.beforecontroljs.position, self.lastjs.position)):
