// copy pasted and slightly altered version of video_recorder.cpp from
// https://github.com/ros-perception/image_pipeline/blob/indigo/image_view/src/nodes/video_recorder.cpp
// I've added service calls to start and stop it, that's it.
...
#include <action_recognition_experiment/Actname.h>
...
bool recording;
int d_recording = 0;
action_recognition_experiment::Actname srv;
...
if (recording)
{
    if (!outputVideo.isOpened()) {
        ...
        ROS_INFO_STREAM("Starting to record " << codec << " video at " << size << "@" << fps << "fps." );
...
bool rec(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res){
  if (recording)
  {
    d_recording = 0;
  }
  ...
  // cond.notify_one();
  ROS_INFO("Started recording now.");
  return true;
}
...
bool set_name(action_recognition_experiment::Actname::Request &req, action_recognition_experiment::Actname::Response &res){
  ROS_INFO_STREAM("Setting filename to: " << req.activity);
  filename = req.activity;
  return true;
}
...
int main(int argc, char** argv)
{
    ...
    ros::ServiceServer service_rec = local_nh.advertiseService("rec", rec);
    ros::ServiceServer service_stop = local_nh.advertiseService("stop", stop);
    ros::ServiceServer service_name = local_nh.advertiseService("set_name", set_name);
